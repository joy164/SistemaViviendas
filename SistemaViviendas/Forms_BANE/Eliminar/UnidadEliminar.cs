...
{
    public partial class UnidadEliminar : Form
    {
        private int noUnidad;
        public UnidadEliminar()
        {
            InitializeComponent();
            LlenarCB();
        }
        ...
        private void btn_accion_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Unidad.DeleteUnidad(noUnidad))
                {
                    MessageBox.Show("La unidad habitacional se elimino con exito", "Baja exitosa", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ValidacionEntradas.LimpiarCampos(tb_nomUnidad, NUD_noDeptos, cb_tipoUnidad, tb_dirUnidad);
